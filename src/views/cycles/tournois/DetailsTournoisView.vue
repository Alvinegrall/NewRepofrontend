  ...
  <div class="container min-h-screen">
    <div class="">
      <div class="mb-3 flex justify-between items-center flex-wrap gap-4">
        ...
        <!-- <div class="flex items-center gap-5">
          ...
          >
            <vue-feather class="h-5" type="save"></vue-feather>
            <div>Enregistrer</div>
          </BaseButton>

          <button
            @click="handleSubmit"
            class="flex items-center gap-2 text-sm bg-red-200 text-red-500 font-[500] shadow-sm hover:scale-x-[1.1] transition-all duration-100 ease-linear mt-6 p-3 rounded-lg"
          >
            <vue-feather class="h-5" type="x-circle"></vue-feather>
            <div>Clôturer</div>
          </button>
    ...
    <div class="mb-10">
      <div class="flex flex-col s901:flex-row gap-10 mb-10">
        ...
        >
          ...
          >
            ...
            <div class="my-10">
              <div
                class="flex w-full justify-between mb-5 border rounded-lg py-3 mt-16"
              >
                <div
                ...
                >
                  ...
                  <div class="font-semibold">
                    {{ formateDate(currentTournoi.date_debut) }}
                  </div>
                </div>
                <div
                ...
                >
                  <div>Date de fin:</div>
                  <div class="font-semibold">
                    {{ formateDate(currentTournoi.date_fin) }}
                  </div>
                </div>
              </div>
              ...
              >
                ...
                <div>
                  ...
                  <div class="text-lg s901:text-xl font-bold">
                    {{ formateDate(currentTournoi.date_debut) }}
                  </div>
                </div>
        ...
        <div class="w-full s901:w-1/3 sticky top-[10%]">
          ...
          >
            <div class="">
              <h3 class="text-lg font-[700] mb-3">Action sur le tournoi</h3>
            </div>
            <div class="">
              <button
                @click="createSeance()"
      ...
      >
        ...
        <div>
          <h3 class="text-lg font-[500] mb-3 border-b">Séances</h3>
          <template v-if="currentTournoi?.seance?.length">
            <div
              v-for="(item, index) in currentTournoi?.seance || []"
              :key="index"
            ...
            >
              ...
              <div>
                <h6 class="text-gray-400 font-semibold text-sm">Location:</h6>
                {{ item.localisation }}
              </div>

              <div>
                <h6 class="text-gray-400 font-semibold text-sm">GPS:</h6>
                <a
                ...
                >
                  <div class="flex">
                    ...
                    <div
                      class="flex justify-center items-center text-white bg-secondary shadow-sm px-3 py-1 rounded-lg gap-2 hover:bg-orange-500 transition-all duration-300 cursor-pointer"
                    >
                      <div>Voir</div>
              ...
              <div v-if="item.status == '1'" class="w-24">
                <h6 class="text-gray-400 font-semibold text-sm">Etat:</h6>
                <button
                  @click="handleSubmit"
                  class="flex items-center gap-2 text-sm bg-green-200 text-green-500 font-[500] shadow-sm hover:scale-x-[1.1] transition-all duration-100 ease-linear px-3 py-1 rounded-lg"
                >
                  <vue-feather class="h-5" type="x-circle"></vue-feather>
                  <div>Clôturer</div>
                </button>
              ...
              <div v-else class="w-24">
                ...
                <button
                  class="flex items-center gap-2 text-sm bg-red-200 text-red-500 font-[500] shadow-sm px-3 py-1 rounded-lg cursor-not-allowed"
                >
                  <vue-feather class="h-5" type="check"></vue-feather>
                  <div>Terminé</div>
                </button>
              </div>
            </div>
...
export default {
  ...
  async mounted() {
    await this.$store
      .dispatch("association/getTournoi", this.$route.params.code)
      .then((response) => {
        console.log("response", response);
        if (!response.data.error) {
          this.currentTournoi = response.data.data;
        }
      })
      .catch((error) => {
        this.$snackbar.add({
  ...
  // computed: {
  //   ...mapGetters("association", ["currentTournoi"]),
  // },
  methods: {
    showDetails(item) {
      console.log(item);
      this.$router.push({
        ...
        params: { code: item.seance_code },
      });
    },
    createSeance() {
      this.$router.push({ name: "cycles.create-seance" });
    },
