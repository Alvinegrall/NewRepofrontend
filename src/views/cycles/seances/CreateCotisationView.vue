          ...
          >
            <form id="create-form" @submit.prevent="handleSubmit($target)">
              ...
              >
                ...
                <!-- <label
                  for=""
                  class=" w-full sm:basis-1/3 "
                >
                  <BaseMemberSelection
                    name="Choisir la seance"
                    :items="membres"
                    :modelvalue="fields.code"
                    @update:modelvalue="fields.code = $event"
                  />
                </label> -->
...
export default {
  ...
  mounted() {
    this.changeDateFormat("12/04/2021 22:00");
  },
  methods: {
    setReceiver(item) {
      this.reciever = item;
    ...
    async handleSubmit() {
      ...
      if (this.fields.is_auto_sanction == "1") {
      ...
      if (this.fields.end_date < this.fields.start_date) {
        this.errors.push(
      ...
      const data = {
        ...
        is_auto_sanction: this.fields.is_auto_sanction,
        amount_sanction: this.fields.amount_sanction,
        motif_sanction: this.fields.motif_sanction,
      };

      await this.$store
        .dispatch("cotisations/createCotisation", data)
        .then(async (response) => {
