<template>
  ...
  >
    <div
      class="modal-dialog modal-dialog-scrollable modal-md relative w-auto pointer-events-none"
    >
      <div
      ...
      >
        ...
        >
          <h5
            id="updateCouponLabel"
            class="text-lg font-bold leading-normal text-gray-800"
          >
            {{ $t("coupons_page.forms.edit-coupon.title") }} "{{
              fields.coupon_code
            }}"
          </h5>
          <button
...
export default {
  ...
  computed: {
    ...mapGetters("coupons", ["current_coupon"]),
  },
  watch: {
      ...
      await CouponsService.getcoupon({ urlcode, coupon: val }).then(
        ({ data }) => {
          if (data.error_msg === "no_coupon") {
            ...
            if (data.coupon.coupon_code === this.current_coupon.coupon_code) {
    ...
    "$route.params.coupon_code": async function (newValue) {
      this.fields.coupon = newValue;
      await store.dispatch("coupons/get_coupon", newValue);

      this.fields.nb_total = this.current_coupon.nb_total;
      this.fields.amount = this.current_coupon.amount;
      this.fields.coupon_code = this.current_coupon.coupon_code;

      // get date on format YYYY-MM-DD
      this.fields.begin_date = this.dateStr(this.current_coupon.date_debut);
      this.fields.end_date = this.dateStr(this.current_coupon.date_fin);
    },
  },
  mounted() {
    this.fields.urlcode = store.getters["auth/urlcode"];
    this.fields.coupon = this.$route.params.coupon_code;

    store
      .dispatch("coupons/get_coupon", this.$route.params.coupon_code)
      .then(() => {
        this.fields.nb_total = this.current_coupon.nb_total;
        this.fields.amount = this.current_coupon.amount;
        this.fields.coupon_code = this.current_coupon.coupon_code;
  ...
  methods: {
    ...
    async handleSubmit() {
      ...
      CouponsService.newcoupon(this.fields)
        .then(async ({ data }) => {
          if (!data.error) {
            this.$snackbar.add({
              text: this.$t("notifications.coupons.update.success"),
              type: "success",
            });

            await store.dispatch("coupons/listcoupons");
            await store.dispatch("coupons/get_coupon", data.coupon.coupon_code);
            this.$router.push({
              name: "coupons.manage.infos",
              params: { coupon_code: data.coupon.coupon_code },
            });
            window.$("#updateCoupon").modal("hide");
          } else {
            this.$snackbar.add({
              text: this.$t("notifications.coupons.update.error"),
              type: "error",
            });
          }
        })
