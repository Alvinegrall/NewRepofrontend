              ...
              <ol class="inline-flex items-center space-x-1 md:space-x-3">
                ...
                <li>
                  <div class="flex items-center">
                    ...
                    <router-link
                      :to="{ name: 'finance.solde-du-compte' }"
                      class="inline-flex items-center text-lg font-semibold text-gray-600 hover:text-gray-600"
                    >
                      Solde de compte
                    </router-link>
              ...
              >
                ...
                >
                  <div class="flex gap-0.5 pb-1">
                    <span class="w-full">Type de compte</span>
                  </div>
                  <div class="text-darkblue flex shrink-0 w-full rounded-lg">
                    ...
                    >
                      ...
                      <option value="1" selected>Compte à terme</option>
                      <option value="0">Compte standard</option>
                    </select>
                  </div>
                </label>
                <label
                ...
                >
                  <div class="flex gap-0.5 pb-1">
                    <span class="w-full">Nombre de temps</span>
                  </div>
                  <div class="text-darkblue flex shrink-0 w-full rounded-lg">
                    <select
                      name="type"
                    ...
                    >
                      <option value="" disabled selected>
                        Choisir le nombre de temps
                      </option>
                      <option v-for="(item,index) in mois" :key="index" :value="item">{{ item }} mois</option>
                    </select>
                  </div>
                </label>
...
export default {
  ...
  methods: {
    ...
    async handleSubmit() {
      ...
      if (this.fields.type === '1' && this.fields.period === "") {
        this.errors.push("Choisir la periode");
      }
      ...
      console.log("datasss",this.fields);

      this.fields.partner_urlcode = urlcode;
      

      await this.$store
        .dispatch("comptes/createCompte", this.fields)
        .then(async (response) => {
          console.log("response add ", response);
          if (!response.data.error) {
            await store.dispatch("comptes/getAllComptes");
            this.$snackbar.add({
              text: "Compte crée avec succès",
              type: "success",
            });
            this.$router.push({
              name: "finance.solde-du-compte",
            });
            this.fields = {
              name:"",
              type:"",
