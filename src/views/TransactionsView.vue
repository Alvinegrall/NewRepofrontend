<template>
  <div class="page-container-wrapper">
            ...
            <li>
              <div class="flex items-center">
                ...
                <span
                  ...
                  >{{ $t("transactions_page.title") }}</span
                >
              </div>
            </li>
          </ol>
    ...
    <div class="row">
          ...
          >
            <div class="media-body space-y-6 w-1/2">
              <div class="text-2xl font-semibold tracking-widest">
                {{ $t("home_page.account.balance") }}
              </div>
              <div v-if="!loading" class="text-4xl font-bold">
                {{ $formatAmount(current_partner.balance) }} XAF
                <span
                  class="d-inline-flex align-items-center badge badge-light-danger grow"
                >
                  <vue-feather :type="kpi.order_count.icon"></vue-feather
                  >{{ kpi.order_count.growth }}%</span
                >
              </div>
              ...
              <div
                class="w-full flex sm:hidden flex-wrap justify-between space-y-4"
              >
                <BaseButton
                  type="button"
                  class="text-base md:text-xl bg-primary px-8 py-2 rounded-lg"
      ...
      <LastTransactionsSection :loading="loading" />
    </div>
  </div>
</template>
